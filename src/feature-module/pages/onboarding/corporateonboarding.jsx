...
const CorporateOnboarding = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setError("Something went wrong. Please check the account number or try again.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <>
          ...
          <div className="card mt-4">
            <div className="card-body">
              ...
              <table className="table table-bordered mt-3">
                ...
                <tbody>
                  {users.map((user, idx) => (
                    <tr key={idx}>
                      <td>
                        <input className="form-control" value={user.full_name} readOnly />
                      </td>
                      <td>
                        <input
                          className="form-control"
                      ...
                      <td>
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            const selectedRole = rolesList.find((r) => String(r.id) === selectedId);
                            handleUserChange(idx, "role_id", selectedId);
                            handleUserChange(idx, "role", selectedRole?.label || "");
                          }}
                        >
              ...
              <button className="btn btn-success" onClick={handleSubmitOnboarding}>
                Submit Onboarding
              </button>
              ...
              {onboardingResult && (
                <div
                  className={`alert mt-4 ${
                    onboardingResult.status === "success" ? "alert-success" : "alert-danger"
                  }`}
                >
